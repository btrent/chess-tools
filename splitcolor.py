...
def main():
    global filename_white
    global filename_black
    global player
    status = 0
    line = ""
    game = {}
    ...
    if len(sys.argv) != 3:
        print "USAGE: splitcolor <input file> <name> \
            \n\nExample: splitcolor test.pgn \"John Doe\"\n"
        sys.exit(0)

    filename = sys.argv[1]
    player = sys.argv[2]
    filename_white = filename.split('.')[0] + '_white.pgn'
    filename_black = filename.split('.')[0] + '_black.pgn'
...
def print_games():
    ...
    filehandlew = open(filename_white, 'w')
    filehandleb = open(filename_black, 'w')
    filehandle = None
